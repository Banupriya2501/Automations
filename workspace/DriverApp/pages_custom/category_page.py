...
class Categorypage(lib.BasePage):
    ...
    def click_product_add(self, product):
        ...
        if product == "chicken":
            ...
            wholeoff = self.driver.find_element_by_xpath('//*[contains(@id,"add-button-CHK_WHL_SKIN_OFF")]')
        ...
        if product == "egg":
            time.sleep(1)
            self.driver.implicitly_wait(15)
            wholeoff = self.driver.find_element_by_xpath('//*[contains(@id,"add-button-CHK_CNTRY_EGG")]')
            action = TouchActions(self.driver)
            action.tap(wholeoff).perform()
        ...
        if product == "pickle":
            ...
            marinadesicon = self.driver.find_element_by_css_selector('.category-row > div#category-item-7')
            action = TouchActions(self.driver)
            action.tap(marinadesicon).perform()
            # self.driver.implicitly_wait(30)
            self.driver.implicitly_wait(15)
            wholeoff = self.driver.find_element_by_xpath('//*[contains(@id,"add-button-PKL_GUNTUR_PRAWN")]')
            action = TouchActions(self.driver)
            action.tap(wholeoff).perform()
